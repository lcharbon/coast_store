...
class FormNavigation extends Component {
	...
	renderPrevArrow() {
		return (
			<div className={ styles["previous-arrow"]} onClick={ this.props.navigateBackward }>
				{ Icons.insert("next_arrow_outline", styles["arrow-icon"]) }
	...
	renderCompleteProcessingLabel() {
		return (
			<div className={ styles["complete-processing-label"]} onClick={ this.props.oncomplete }>
				<div className={ styles["caption"]}>
					{ $T(66) /* Processing */ }
				</div>
			</div>
	...
	render() {
		return (
			<div className={ styles["navigation"] }>
				...
				{ this.props.oncomplete && ( this.props.isProcessing ? this.renderCompleteProcessingLabel() : this.renderCompleteButton() ) }
			</div>
		)
	}
}
