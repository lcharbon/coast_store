...
class Fulfilment extends Model {
	...
	compileShipmentData() {
		return {
			...
			"extra": { 
					"reference_1": this.generateOrderSummary()
			},
			"async": false
		}
	}
	...
	findBestValueRate(rates) {
		var bestValueRate;
		var bestweightedPrice = 0

		bestValueRate = rates.filter(function(rate) { return rate.attributes.includes("BESTVALUE") })[0];

		if (!bestValueRate) {
			rates.forEach(function(rate) {
				var weightPrice = 0;

				if (rate.estimated_days !==0 && !rate.estimated_days) return;

				weightPrice = parseFloat(rate.amount) + rate.estimated_days * 10;

				if (bestweightedPrice === 0 || weightPrice < bestweightedPrice) {
					bestweightedPrice = weightPrice;
					bestValueRate = rate;
				}
			})
		}

		return bestValueRate;
	}
	...
	generateOrderSummary() {
		var purchase;
		var purchasesSummary = [];
		var summaryString = "";
		
		for (purchase of this.order.purchases) {
			purchasesSummary.push(`|${$T(purchase.variant.color)} ${purchase.variant.size}(${purchase.quantity})|`);
		}

		summaryString = purchasesSummary.join(" ");

		if (summaryString.length > 45) return "";
